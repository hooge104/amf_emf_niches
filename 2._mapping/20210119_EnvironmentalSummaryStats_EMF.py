...
predictedProbs = ee.ImageCollection('projects/crowtherlab/johan/2021_fungi_SDMs/EMF_probabilities');

SH_list = list(pd.read_csv('/Users/johanvandenhoogen/ETH/Projects/AMF/data/EMF_listToModel.csv')['unite_sh'])
threshold_list = [0.25, 0.5, 0.75, 0.9, 0.95]

mapList = []
for item in threshold_list:
    mapList = mapList + (list(zip(SH_list, repeat(item))))

# SH_list, threshold_list = zip(*mapList)
...
def summary_func(taxon, threshold):
    ...
    while not success:
            try:
                ...
                with open('output/threshold_' + str(threshold) + '_EMF_reducedFeatures.csv', 'a') as f:
