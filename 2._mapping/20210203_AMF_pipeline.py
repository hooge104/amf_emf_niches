...
# subList = [s for s in subList if any(xs in s for xs in ['RUNNING','READY'])]
# runningList = list(map(lambda s: s.strip("<Task EXPORT_IMAGE: ").strip("_probability (READY)>"), subList))
#
# taxonList = [x for x in fullTaxonList if x not in runningList]
...
def pipeline(taxonOfInterest):
    ...
    while not success:
            try:
                ...
                projectFolder = '2021_AMF/'+taxonOfInterest
            ...
            #                             variablesPerSplit=2,
                ...
                def varImpFunc(fc):
                    ...
                    Path('output/feature_importances/AMF').mkdir(parents=True, exist_ok=True)

                    with open('output/feature_importances/AMF/'+taxonOfInterest+'_featureImportances.csv', 'a') as f:
